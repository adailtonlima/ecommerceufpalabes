package br.ufpa.facomp.labes.web.rest;

import static br.ufpa.facomp.labes.web.rest.TestUtil.sameInstant;
import static org.assertj.core.api.Assertions.assertThat;
import static org.hamcrest.Matchers.hasItem;
import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.*;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.*;

import br.ufpa.facomp.labes.IntegrationTest;
import br.ufpa.facomp.labes.domain.Endereco;
import br.ufpa.facomp.labes.domain.Usuario;
import br.ufpa.facomp.labes.repository.UsuarioRepository;
import br.ufpa.facomp.labes.service.criteria.UsuarioCriteria;
import br.ufpa.facomp.labes.service.dto.UsuarioDTO;
import br.ufpa.facomp.labes.service.mapper.UsuarioMapper;
...
class UsuarioResourceIT {
    ...
    @Test
    @Transactional
    void getAllUsuariosByMeusEnderecosIsEqualToSomething() throws Exception {
        // Initialize the database
        usuarioRepository.saveAndFlush(usuario);
        Endereco meusEnderecos;
        if (TestUtil.findAll(em, Endereco.class).isEmpty()) {
            meusEnderecos = EnderecoResourceIT.createEntity(em);
            em.persist(meusEnderecos);
            em.flush();
        } else {
            meusEnderecos = TestUtil.findAll(em, Endereco.class).get(0);
        }
        em.persist(meusEnderecos);
        em.flush();
        usuario.addMeusEnderecos(meusEnderecos);
        usuarioRepository.saveAndFlush(usuario);
        Long meusEnderecosId = meusEnderecos.getId();

        // Get all the usuarioList where meusEnderecos equals to meusEnderecosId
        defaultUsuarioShouldBeFound("meusEnderecosId.equals=" + meusEnderecosId);

        // Get all the usuarioList where meusEnderecos equals to (meusEnderecosId + 1)
        defaultUsuarioShouldNotBeFound("meusEnderecosId.equals=" + (meusEnderecosId + 1));
    }
