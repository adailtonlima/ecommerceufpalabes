...
import br.ufpa.facomp.labes.repository.PedidoRepository;
import br.ufpa.facomp.labes.service.criteria.PedidoCriteria;
import br.ufpa.facomp.labes.service.dto.PedidoDTO;
import br.ufpa.facomp.labes.service.mapper.PedidoMapper;
...
class PedidoResourceIT {
    ...
    @Test
    @Transactional
    void getAllPedidosByProdutosIsEqualToSomething() throws Exception {
        // Initialize the database
        pedidoRepository.saveAndFlush(pedido);
        ProdutoNoPedido produtos;
        if (TestUtil.findAll(em, ProdutoNoPedido.class).isEmpty()) {
            produtos = ProdutoNoPedidoResourceIT.createEntity(em);
            em.persist(produtos);
            em.flush();
        } else {
            produtos = TestUtil.findAll(em, ProdutoNoPedido.class).get(0);
        }
        em.persist(produtos);
        em.flush();
        pedido.addProdutos(produtos);
        pedidoRepository.saveAndFlush(pedido);
        Long produtosId = produtos.getId();

        // Get all the pedidoList where produtos equals to produtosId
        defaultPedidoShouldBeFound("produtosId.equals=" + produtosId);

        // Get all the pedidoList where produtos equals to (produtosId + 1)
        defaultPedidoShouldNotBeFound("produtosId.equals=" + (produtosId + 1));
    }
